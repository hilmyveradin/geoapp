    ...
    if (mapLoaded && map) {
      ...
      const updateLayers = () => {
        ...
        layerOrderRef.current.forEach((layerId) => {
          ...
          const url = `${process.env.NEXT_PUBLIC_BASE_URL}/geoserver/wms?service=WMS&version=1.1.0&request=GetMap&layers=${layer.layerName}&bbox={bbox-epsg-3857}&width=512&height=512&srs=EPSG:3857&styles=&format=image%2Fpng&transparent=true&token=${session.accessToken}`;
