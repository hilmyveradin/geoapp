...
export async function POST(request) {
  try {
    ...
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_BASE_URL}${process.env.NEXT_PUBLIC_GEOPORTAL_PATH}/be/cms/layer/${layerUid}/changeprop`,
      {
        method: "POST",
        headers: {
    ...
    if (!res.ok) {
      ...
      return new NextResponse(`session: ${session}`, { status: res.status });
    }
