...
const ChangePropDialog = ({
  ...
  const changePropAction = () => {
    const changeMapProp = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_GEOPORTAL_PATH}/api/maps/change-prop?mapUid=${objectUid}`,
          {
            method: "POST",
    ...
    const changeLayerprop = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_GEOPORTAL_PATH}/api/layers/change-prop?layerUid=${objectUid}`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(body),
