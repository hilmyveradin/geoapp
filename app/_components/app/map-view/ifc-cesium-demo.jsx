...
const IFCViewer = () => {
  ...
  useEffect(() => {
    const initializeViewer = async () => {
      ...
      worldRef.current.renderer = new OBC.SimpleRenderer(
  ...
  const exportFragments = async () => {
    if (!fragmentsRef.current.groups.size) {
      return;
    }
    const group = Array.from(fragmentsRef.current.groups.values())[0];
    const data = fragmentsRef.current.export(group);
    download(new File([new Blob([data])], "small.frag"));
