...
const MapSidebar = () => {
  ...
  const Button = ({ icon, label, onClick }) => (
    <ButtonSidebar variant="ghost" onClick={onClick}>
      {icon}
      {!showSidebar && <span className="ml-2 inline-block">{label}</span>}
    </ButtonSidebar>
  );
  ...
  // Button data
  const buttons = [
    { icon: <PlusCircle className={cn("w-4 h-4")} />, label: 'Add', onClick: () =>  handleButtonClick('add')},
    { icon: <Layers3 className={cn("w-4 h-4")} />, label: 'Layers', onClick: () =>  handleButtonClick('layers')},
    { icon: <img className={cn("w-4 h-4")} src="/app/basemap-svgrepo-com.svg" alt="basemap icon" />, label: 'Basemap', onClick: () =>  handleButtonClick('basemap') },
    { icon: <Sheet className={cn("w-4 h-4")} />, label: 'Tables', onClick:  () =>  handleButtonClick('tables') },
    { icon: <Save className={cn("w-4 h-4")} />, label: 'Save', onClick:  () =>  handleButtonClick('save') },
    { icon: <Share2 className={cn("w-4 h-4")} />, label: 'Share', onClick:  () =>  handleButtonClick('share') },
    { icon: <Printer className={cn("w-4 h-4")} />, label: 'Print', onClick:  () =>  handleButtonClick('print') },
    { icon: <ChevronLeft className={cn("w-4 h-4", { "-rotate-180": showSidebar })} />, label: 'Collapse', onClick: () => setShowSidebar(prev => !prev) },
  ];
